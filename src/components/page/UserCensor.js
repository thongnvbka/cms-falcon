...
const UserCensor = () => {
    ...
    },
    {
      ...
      formatter: (cellContent, row) => {
        ...
        if (true) {
          return (
            <>
              <Button onClick={()=>hanldeCensor(row.user._id,true)} color="falcon-success" size="sm" className="ml-2">
                Đồng ý
              </Button>
              <Button onClick={()=>hanldeCensor(row.user._id,false)} color="falcon-danger" size="sm" className="ml-2">
                Từ chối
              </Button>
            </>
          );
  ...
  const getGameList = async () => {
    const data = await censorAPI.getListCensor(1, 1000);
  ...
  return (
    <Card className="mb-3">
      <FalconCardHeader title="Danh sách kiểm duyệt" light={false}>
        {isSelected ? (
