...
const Games = () => {
    ...
    return (
      <Link to={`/games/detail/${id}`}>
        <Media tag={Flex} align="center">
          <Avatar src={urlConfig.BASE_URL_FILE_MANAGER + '/uploads/' + image} />
  ...
  const imageFormatter = (dataField, { backgroud, id }) => {
    return (
      <Link to={`/games/detail/${id}`}>
        <Avatar src={urlConfig.BASE_URL_FILE_MANAGER + '/uploads/' + backgroud} />
      </Link>
    )
  };

  const statusFormatter = (dataField, { id, isActive }) => {
    return (
      <Badge
        onClick={() => changeStatusGame(id, isActive)}
        className="btn"
  ...
  const changeStatusGame = async (id, isActive) => {
    if (window.confirm('Are you sure to change status game?')) {
      if (isActive) {
        const result = await gamesAPI.inactive(id);
      } else {
        const result = await gamesAPI.active(id);
      }
      getGameList();
      toast(
        <Fragment>
          <h6>Message</h6>
          <hr />
          <p className="mb-0">Change status game successfully!</p>
        </Fragment>
      );
    }
  }
