...
const Companion = () => {
  const hanldeCensor = async ( userId, status ) => {
    const resp = await censorAPI.handelCensor({userId,status});
    if(resp){
      alert('sucsess')
    }
  };
  ...
  const actionFormatter = (cell,row) => (
    <UncontrolledDropdown>
      ...
      <DropdownMenu right className="border py-2">
        <DropdownItem onClick={() => editCompanion(row)}>Edit</DropdownItem>
        <DropdownItem onClick={() => deleteCompanion(row._id)} className="text-danger">
          Delete
        </DropdownItem>
      </DropdownMenu>
    </UncontrolledDropdown>
  ...
  const getListCompanion = async () => {
    const data = await systemAPI.getListCompanion();
  ...
  const deleteCompanion = async (id) =>{
    const resp =  await systemAPI.deleteCompanion(id)
    if(resp){
      toast(
        <Fragment>
          <hr />
          <p className="mb-0">Xoá thành công!</p>
        </Fragment>
      );
      setReload(!reload)
    }
  ...
  return (
    <Card className="mb-3">
      <FalconCardHeader title="Danh sách nhà đồng hành" light={false}>
        {isSelected ? (
      ...
      <CardBody className="p-0">
        ...
        <ModalCom modal={modal} reload={setReload} setModal={setModal} type={type} modalContent={dataModal} />
      </CardBody>
    </Card>
  );
};
